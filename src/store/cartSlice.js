...
export const cartSlice = createSlice({
  ...
  reducers: {
    addProductToCart: (state, action) => {
      ...
      const existingIndex = state.orderedProducts.findIndex(
    ...
    decreaseOrderedProducts(state, action) {
      const itemIndex = state.orderedProducts.findIndex(
        (item) => item.id === action.payload.id
      );
      ...
      if (state.orderedProducts[itemIndex].cartQuantity > 1) {
        ...
        const nextCartItems = state.orderedProducts.filter(
          (item) => item.id !== action.payload.id
        );
    ...
    removeProductFromCart: (state, action) => {
      state.orderedProducts.map((orderedProducts) => {
        if (orderedProducts.id === action.payload.id) {
          const updateOrderedProducts = state.orderedProducts.filter(
            (item) => item.id !== orderedProducts.id
          );
